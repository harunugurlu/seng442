...
void search(int age, int * node_array, int index, int my_rank, double start_time)
{
    if (index == -1 || target_found) return;
    ...
    if (node_array[index * 4 + AGE] == age)
    {
        ...
        printf("Time: %lf, process id: %d numtasks: %d\n", time, my_rank, MPI_COMM_WORLD);
    ...
    {
        search(age, node_array, node_array[index * 4 + LEFT], my_rank, start_time);
    }
    else
    {
        search(age, node_array, node_array[index * 4 + RIGHT], my_rank, start_time);
    }
}
